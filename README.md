...
- 参照音声: クリーンな単一話者の wav（10〜30 秒程度）．`reference/clone.wav` に置く
...
1. `reference audio` に参照音声のパスが入っていることを確認（初回は `reference/clone.wav` が設定済み）
...
5. reference audio に参照音声（このリポジトリの `reference/clone.wav` に相当するファイル）を指定して Start
...
reference/clone.wav           # 参照音声（クローン先の声）
